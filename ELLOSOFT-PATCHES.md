...
- `runTask()`: immediately BEFORE the `backend, err := agent.New(...)` call, inserted the sandbox-redirect block. It computes `execPathForAgent` (default `entry.Path`), and when `d.sandboxMgr != nil && task.ProjectID != ""` and the server's `GetProjectSandboxConfig` returns an enabled config, it calls `EnsureSandbox(ctx, task.IssueID, ...)` (mounting WorkspacesRoot read-write, the agent bin dir + multica self-bin dir read-only, and TempDir read-write), writes an `agent-shim.sh` wrapper into `filepath.Dir(env.WorkDir)` (the env root, outside the git worktree) that execs `multica __sandbox-exec`, and sets `execPathForAgent = shimPath`. When a container is used it also calls `d.sandboxMgr.TaskStarted(task.IssueID)` and `defer`s `KillTaskProcess(context.Background(), cid, task.ID)` + `TaskFinished(task.IssueID)` — so the reaper never removes a container with an in-flight task, and the in-container agent process is reliably terminated from the daemon on cancellation (the shim is SIGKILLed and cannot self-clean). Config-fetch errors fall back to host execution (warn-only); ensure/write errors return `TaskResult{}, fmt.Errorf(...)`. The only change to the `agent.New` call is `ExecutablePath: execPathForAgent`.
- `Run()`: next to the other `go d.…Loop(ctx)` launches (after `go d.serveHealth(...)`), added a guarded block (only when `d.sandboxMgr != nil`) that first calls `d.sandboxMgr.Adopt(ctx)` (seeds last-used for sandbox containers that survived a daemon restart so they're reaped, not leaked) then starts `go d.sandboxMgr.ReapLoop(ctx, 10*time.Minute, d.sandboxIdleTTL)`.
